...
export class DingTalkService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      this.accessTokenExpireTime =
        now + ((data.expires_in || 7200) * 1000 - 60000);
  ...
  private async sendMessage(
    ...
    if (params.openConversationId)
      queryParams.set("openConversationId", params.openConversationId);
   ...
   */
  async sendMarkdownToConversation(
    conversationId: string,
    title: string,
    text: string,
  ): Promise<SendMessageResponse> {
