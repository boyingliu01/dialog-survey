...
| 层级 | 职责 | 技术选型 | 关键文件 |
|------|------|----------|----------|
| Access Layer | 接收钉钉消息、签名验证、语音转文字 | FastAPI, Fun-ASR | `src/api/webhook.py`, `src/services/asr.py` |
| Application Layer | 对话状态管理、业务逻辑 | LangGraph, Python | `src/core/graph.py`, `src/core/nodes.py` |
| AI Service Layer | LLM 调用、Prompt 管理 | OpenAI-compatible API | `src/services/llm.py`, `src/services/prompts.py` |
| Storage Layer | 数据持久化 | PostgreSQL, SQLAlchemy | `src/models/` |
...
**实现方案**:
```python
# src/core/state.py - 新增字段
class InterviewState(TypedDict):
    # ... existing fields
    followup_count: dict[str, int]  # {topic_id: count}

# src/core/nodes.py - interviewing_node 修改
def interviewing_node(state: InterviewState) -> dict:
    topic_id = state["current_topic"]
    followup_count = state.get("followup_count", {})
    
    if followup_count.get(topic_id, 0) >= 2:
        # 跳过追问，直接下一话题
...
**实现方案**:
```python
# src/services/prompts.py - 优化报告 Prompt
REPORT_GENERATE_PROMPT_V2 = """
基于以下访谈内容，生成结构化报告：
...
**实现方案**:
```python
# src/services/report_service.py (新文件)
class ReportService:
...
**实现方案**:
```python
# src/services/dingtalk_sender.py (新文件)
...
class InterviewPlan(Base):
    ...
    # Relationships
    interviewees = relationship("Interviewee", back_populates="plan")

class Interviewee(Base):
    __tablename__ = "interviewees"
    
    id = Column(Integer, primary_key=True)
    plan_id = Column(Integer, ForeignKey("interview_plans.id"))
    user_id = Column(String(100))  # 钉钉用户 ID
    name = Column(String(100))
    phone = Column(String(20))
    status = Column(String(20), default="pending")  # pending, invited, in_progress, completed
...
class AnalysisJob(Base):
    ...
    total_interviews = Column(Integer)              # 待分析的访谈总数
    ...
    # Relationships
    plan = relationship("InterviewPlan", back_populates="analysis_jobs")
    topics = relationship("AnalysisTopic", back_populates="job")
...
class AnalysisTopic(Base):
    ...
    name = Column(String(255))                      # 主题名称
    keywords = Column(JSON)                         # 关联关键词列表
    mention_count = Column(Integer)                 # 提及次数
    mention_rate = Column(Float)                    # 提及率 (百分比)
    positive_ratio = Column(Float)                  # 正面评价占比
    negative_ratio = Column(Float)                  # 负面评价占比
    neutral_ratio = Column(Float)                   # 中性评价占比
    created_at = Column(DateTime, default=datetime.utcnow)
    
    # Relationships
    job = relationship("AnalysisJob", back_populates="topics")
    key_points = relationship("KeyPoint", back_populates="topic")
...
class KeyPoint(Base):
    ...
    id = Column(Integer, primary_key=True)
    topic_id = Column(Integer, ForeignKey("analysis_topics.id"))
    interview_id = Column(Integer, ForeignKey("interviews.id"))
    summary = Column(Text)                          # 观点摘要
    quote = Column(Text)                            # 原文引用
    sentiment = Column(String(20))                  # positive, negative, neutral
    interviewee_role = Column(String(100))          # 回答者角色/身份
    created_at = Column(DateTime, default=datetime.utcnow)
...
class AnalysisResult(Base):
    ...
    id = Column(Integer, primary_key=True)
    job_id = Column(Integer, ForeignKey("analysis_jobs.id"))
    total_interviews = Column(Integer)              # 分析的访谈总数
    overall_sentiment = Column(JSON)                # 整体情感分布
    top_topics = Column(JSON)                       # Top 10 主题列表
    satisfaction_score = Column(Float)              # 整体满意度得分
    nps_score = Column(Float)                       # NPS 得分（可选）
    report_path = Column(String(255))               # 分析报告文件路径
    created_at = Column(DateTime, default=datetime.utcnow)
