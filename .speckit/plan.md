...
| 钉钉 API 限流  | 消息队列缓冲，速率控制 20msg/s     | 队列最大1000条                |
| LLM API 限流   | 令牌桶算法限速 + 消息队列缓冲      | 每秒 10 次调用，队列最大500条 |
...
- 定期巡检: 每5分钟检查数据库连接 + LLM 缓存有效性
- **LLM 健康检查优化**: 不每次调用 API 产生费用，而是：
  1. 检查 AccessToken 缓存有效性（时间戳验证）
  2. 缓存最近一次 LLM 成功调用的时间戳
  3. 如果超过 5 分钟未调用 LLM，则在下一次调用时使用轻量接口（如 model list API）验证
  4. 降级: LLM 完全不可用时返回 degraded 状态但服务仍可用（本地缓存模板）
